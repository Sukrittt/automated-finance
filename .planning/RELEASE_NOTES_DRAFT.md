...
- Dashboard, Transactions, Review Queue, and Insights now support preview fallback mode when live data is unavailable.
- Transactions screen now supports quick debit/credit filtering chips and filter-aware empty states.
- Transactions list now consistently renders newest-first.
...
- Insights ready state now shows a `Last updated` freshness timestamp.
- Ranked top spending categories are now visible in dashboard and insights views.
- Monthly budget setup/edit with local persistence for category limits.
- Review-driven category corrections now persist across app restarts.
- Review queue recategorization now includes quick category chips for faster edits.
